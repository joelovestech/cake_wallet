...
abstract class ZcashWalletBase
  ...
  Future<void> _oneshotSync() async {
    try {
      ...
      final accountList = accounts.map((final a) => a.id)
        .toList()
      ...
      await sync
        ..listen(
          (final syncProgress) {
            ...
            printV("[${c.account} ($accountList)] [$oneshotSyncCount/$randInt] sync: ${syncProgress.height}");
          ...
          onError: (final e) {
            ...
            syncStatus = FailedSyncStatus(error: e);
            isSyncing = false;
          },
          onDone: () {
  ...
  @override
  Future<PendingTransaction> createTransaction(final Object credentials) => _createTransaction(credentials);

  Future<PendingTransaction> _createTransaction(final Object credentials, {final int tryReduceFeeAmount = 0}) async {
    ...
    for (final output in creds.outputs) {
      ...
      BigInt amt = BigInt.from(output.formattedCryptoAmount ?? -1);
      if (output.sendAll) {
        amt = BigInt.from(zcashBalance?.confirmed ?? -1);
        amt -= BigInt.from(tryReduceFeeAmount);
      }
      recipients.add(
    ...
    final sendAmount = creds.outputs.map((final out) => out.formattedCryptoAmount ?? 0).reduce((final a, final b) => a + b);

    if (zcashBalance?.confirmed == sendAmount) {
    ...
    try {
      ...
      return PendingZcashTransaction(
        ...
        credentials: creds,
        txPlan: txPlan,
        fee: tryReduceFeeAmount != 0 ? tryReduceFeeAmount : internalCalculateEstimatedFee(creds.priority, null),
  ...
  Future<void> _initKeys() async {
    try {
      ...
      keys = {
        "privateViewKey": e.toString(),
      };
    }
    try {
      final seedPhrase = (await zkool_account.getAccountSeed(
        account: accountId,
        c: c,
      ))?.phrase.split(" ");
  ...
  Future<void> updateBalance() async {
    try {
      ...
      balance[CryptoCurrency.zec] = ZcashBalance(
        confirmed: confirmedSpendable.toInt(),
        unconfirmed: (confirmedTotal - confirmedSpendable).toInt(),
        frozen: 0,
      );
    } catch (e, stackTrace) {
